...
module.exports = {
  name: 'hack',
  description: 'Hack profile and create an image with custom text',
  author: 'John Lester',
  nashPrefix: true,
...
async function runCommand(api, event, args) {
  ...
  // Use getUserInfo to fetch user data (name)
  const userInfo = await api.getUserInfo(id);
  const name = userInfo[id].name;

  const threadInfo = await api.getThreadInfo(event.threadID);

  if (!name) return api.sendMessage("Enter the content of the comment on the board", threadID, messageID);
